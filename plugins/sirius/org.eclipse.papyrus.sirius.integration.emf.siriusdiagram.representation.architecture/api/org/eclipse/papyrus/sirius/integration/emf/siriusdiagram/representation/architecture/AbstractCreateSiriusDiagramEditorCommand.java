...
import org.eclipse.papyrus.sirius.integration.emf.siriusdiagram.representation.architecture.commands.CreateSiriusDiagramEditorViewCommand;
...
/**
 * This class provides useful method to create a new DDiagram and open its editor
 */
public abstract class AbstractCreateSiriusDiagramEditorCommand implements ICreateSiriusDiagramEditorCommand {
	 ...
	 *
	 * @param dDiagram
	 *            the document template prototype used to create the {@link SiriusDiagram}
	 * @param documentName
	 *            the name of the created document
	 * @param semanticContext
	 *            the semantic context used for the creation of the {@link SiriusDiagram}
	 * @param openAfterCreation
	 *            if <code>true</code> the editor will be opened after the creation
	 * @return
	 *         the created {@link SiriusDiagram}
	 */
	protected SiriusDiagramPrototype execute(final SiriusDiagramPrototype dDiagram, final String documentName, final EObject semanticContext, final boolean openAfterCreation) {
		return execute(dDiagram, documentName, semanticContext, semanticContext, openAfterCreation);
	}
	 ...
	 *            the graphical context used for the creation of the {@link SiriusDiagram}
	 * @param openAfterCreation
	 *            if <code>true</code> the editor will be opened after the creation
	 * @return
	 *         the created {@link SiriusDiagram}
	 */
	protected SiriusDiagramPrototype execute(final SiriusDiagramPrototype docProto, final String documentName, final EObject semanticContext, final EObject graphicalContext, final boolean openAfterCreation) {
		...
		final CreateSiriusDiagramEditorViewCommand command = createSiriusDiagramEditorCreationCommand(domain, docProto, documentName, documentMainTitle, semanticContext, openAfterCreation);
	 ...
	 *         the created {@link SiriusDiagram}
	 */
	public CreateSiriusDiagramEditorViewCommand createSiriusDiagramEditorCreationCommand(final TransactionalEditingDomain editingDomain,
	 ...
	 */
	public CreateSiriusDiagramEditorViewCommand createSiriusDiagramEditorCreationCommand(final TransactionalEditingDomain editingDomain,
			final SiriusDiagramPrototype docProto,
			final String documentName,
			final String documentMainTitle,
			final EObject semanticContext,
			final boolean openAfterCreation) {
		return new CreateSiriusDiagramEditorViewCommand(editingDomain, docProto, documentName, documentMainTitle, semanticContext, openAfterCreation);
	}
	 ...
	 *
	 * @param semanticContext
	 *            the semantic context for the create SiriusDiagram
	 * @return
