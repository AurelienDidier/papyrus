...
import org.eclipse.papyrus.sirius.integration.emf.siriusdiagram.modelexplorer.internal.commands.RenameSiriusDiagramLabelCommand;
import org.eclipse.papyrus.sirius.integration.emf.siriusdiagram.modelexplorer.internal.messages.Messages;
...
public class RenameSiriusDiagramHandler extends AbstractSiriusDiagramCommandHandler {
	...
	protected Command getCommand(IEvaluationContext context) {
		...
		List<DDiagram> documents = getSelectedSiriusDiagrams();
		if (editingDomain != null && documents.size() == 1) {

			final DDiagram document = documents.get(0);
			...
			if (null != documentLabel && LabelInternationalizationPreferencesUtils.getInternationalizationPreference(document)) {
				AbstractTransactionalCommand cmd = new RenameSiriusDiagramLabelCommand(editingDomain, "ChangeSiriusDiagramLabelCommand", document, documentLabel, Messages.RenameSiriusDiagramHandler_Label_DialogTitle); //$NON-NLS-1$
				return new GMFtoEMFCommandWrapper(cmd);
			} else {
				final String currentName = document.getName();
				if (null != currentName) {
					EObject obj = document.eContainer();
					if (obj instanceof DRepresentation) {
						EStructuralFeature nameFeature = ViewpointPackage.eINSTANCE.getDRepresentation_Name();
						return new RenameElementCommand(editingDomain, "RenameSiriusDiagramCommand", document, currentName, nameFeature, Messages.RenameSiriusDiagramHandler_RenameAnExistingSiriusDiagram, Messages.RenameSiriusDiagramHandler_NewName);
						// //$NON-NLS-1$
					}
				}
			}
