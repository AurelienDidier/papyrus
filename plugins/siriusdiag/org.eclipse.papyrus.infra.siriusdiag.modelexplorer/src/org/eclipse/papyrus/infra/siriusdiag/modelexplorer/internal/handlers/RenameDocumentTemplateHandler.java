 ...
 *
 */
public class RenameDocumentTemplateHandler extends AbstractSiriusDiagramCommandHandler {
	...
	protected Command getCommand(IEvaluationContext context) {
		...
		if (editingDomain != null && documents.size() == 1) {
			...
			if (null != documentLabel && LabelInternationalizationPreferencesUtils.getInternationalizationPreference(document)) {
				AbstractTransactionalCommand cmd = new RenameSiriusDiagramLabelCommand(editingDomain, "ChangeDocumentTemplateLabelCommand", document, documentLabel, Messages.RenameDocumentTemplateHandler_Label_DialogTitle); //$NON-NLS-1$
